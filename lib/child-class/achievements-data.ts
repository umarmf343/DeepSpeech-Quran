...
export interface AchievementDefinition {
  ...
  icon: string
  points: number
  condition: (progress: ChildProgress) => boolean
}
...
export function getUnlockedAchievements(progress: ChildProgress): AchievementDefinition[] {
  return ACHIEVEMENT_DEFINITIONS.filter((achievement) => achievement.condition(progress))
}

export function getNextAchievements(progress: ChildProgress): AchievementDefinition[] {
